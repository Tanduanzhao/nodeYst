...
export default class BreedSingle extends Component{
	render(){
        ...
        if(this.props.data){
            ...
            return(
                <Link to={`/market/marketSearch/marketSearchDetail/${encodeURIComponent(encodeURIComponent(this.props.data.genericName))}/${this.props.data.id}`} className="row item" style={{ padding: '10px',fontSize: '.6rem'}}>
